...
export class CountrySelector extends React.Component<Props> {
  onChange = (formik: *) => (countries: Array<string>) => {
    formik.form.setFieldValue(formik.field.name, countries[0]);
  ...
  render(): React$Node {
    return (
      <FormField
        ...
        render={formik => {
          const country: ?{ name: string } = DATA.find(
            row => row.code === formik.field.value
          );
