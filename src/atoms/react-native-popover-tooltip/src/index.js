...
class PopoverTooltip extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <Animated.View
          style={[
            { opacity: this.state.opposite_opacity },
        ...
        >
              ...
              <Animated.View
                style={[
                  ...
                  this.props.tooltipContainerStyle,
                  {
                    ...
                    transform: [{ scale: this.state.tooltip_container_scale }],
                  },
                ]}
              >
                <View
                  onLayout={ev => {
                    let tooltip_container_width = ev.nativeEvent.layout.width,
                      tooltip_container_height = ev.nativeEvent.layout.height;
                    if (
                      this.state.will_popup &&
                      tooltip_container_width > 0 &&
                      tooltip_container_height > 0
                    ) {
                      this._component_wrapper.measure(
                        (x, y, width, height, pageX, pageY) => {
                          let tooltip_container_x_final =
                            pageX +
                              tooltip_container_width +
                              (width - tooltip_container_width) / 2 >
                            window.width
                              ? window.width - tooltip_container_width
                              : pageX + (width - tooltip_container_width) / 2;
                          let tooltip_container_y_final = this.state
                            .tooltip_triangle_down
                            ? pageY - tooltip_container_height - 20
                            : pageY + tooltip_container_height - 20;
                          let tooltip_triangle_down = this.state
                            .tooltip_triangle_down;
                          ...
                          if (
                            pageY + tooltip_container_height + 80 >
                            window.height
                          ) {
                          ...
                          let button_component_container_scale = this.state.tooltip_container_scale.interpolate(
                            {
                              ...
                              outputRange: [
                                1,
                                this.props.buttonComponentExpandRatio,
                              ],
                            }
                          );
                          ...
                          this.setState(
                            {
                              ...
                              width: width,
                              height: height,
                              tooltip_container_x: tooltip_container_x,
                              tooltip_container_y: tooltip_container_y,
                              tooltip_triangle_down: tooltip_triangle_down,
                              tooltip_triangle_left_margin:
                                pageX +
                                width / 2 -
                                tooltip_container_x_final -
                                10,
                              button_component_container_scale: button_component_container_scale,
                            },
                            () => {
                ...
                >
                  {this.state.tooltip_triangle_down ? null : (
                    <View
                      style={[
                        styles.triangle_up,
                        {
                          marginLeft: this.state.tooltip_triangle_left_margin,
                          left: this.state.triangleOffset,
                        },
                        this.props.labelContainerStyle
                          ? {
                              borderBottomColor: this.props.labelContainerStyle
                                .backgroundColor,
                            }
                          : null,
                      ]}
                  ...
                  {this.state.tooltip_triangle_down ? (
                    <View
                      style={[
                        ...
                        this.props.labelContainerStyle
                          ? {
                              borderTopColor: this.props.labelContainerStyle
                                .backgroundColor,
                            }
                          : null,
                      ]}
          ...
          <Animated.View
            style={[
              ...
              {
                transform: [
                  { scale: this.state.button_component_container_scale },
                ],
              },
            ]}
          >
  ...
  _showZoomingInAnimation() {
    let tooltip_animation = Animated.timing(
      this.state.tooltip_container_scale,
      {
        toValue: 1,
        duration:
          this.props.timmingConfig && this.props.timmingConfig.duration
            ? this.props.timmingConfig.duration
            : 200,
      }
    );
    if (this.props.animationType == 'spring') {
      tooltip_animation = Animated.spring(this.state.tooltip_container_scale, {
        toValue: 1,
        tension:
  ...
  _showZoomingOutAnimation() {
    Animated.parallel([
      Animated.timing(this.state.tooltip_container_scale, {
        toValue: 0,
        duration: this.props.opacityChangeDuration
          ? this.props.opacityChangeDuration
          : 200,
